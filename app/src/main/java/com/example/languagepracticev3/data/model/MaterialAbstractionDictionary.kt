...
object MaterialAbstractionDictionary {
     ...
     * テーマから禁止ワードを生成
     */
    fun generateForbiddenWords(theme: String): List<String> {
        // 基本的な抽象語の禁止リスト
        ...
        // テーマに関連する類義語を追加
        val synonyms = when (theme.lowercase()) {
            "孤独" -> listOf("寂しさ", "独り", "ぼっち", "孤立", "疎外", "一人ぼっち", "淋しい")
            "期待" -> listOf("望み", "願い", "待望", "期する", "楽しみ", "わくわく", "待ち遠しい")
            "喪失" -> listOf("失う", "なくす", "喪う", "失敗", "消失", "紛失", "失った")
            "信頼" -> listOf("信じる", "頼る", "依存", "任せる", "委ねる", "信用")
            "不安" -> listOf("心配", "恐れ", "怖い", "懸念", "危惧", "おびえる", "怯える")
            "希望" -> listOf("望み", "願い", "夢", "光", "明るい", "未来", "願望")
            "後悔" -> listOf("悔やむ", "残念", "惜しい", "もったいない", "悔しい")
            "焦燥" -> listOf("焦り", "いらいら", "もどかしい", "じれったい", "焦る")
            "儚さ" -> listOf("儚い", "はかない", "脆い", "もろい", "一瞬", "刹那")
            "懐かしさ" -> listOf("懐かしい", "なつかしい", "思い出", "追憶", "郷愁", "ノスタルジー")
            "悲しみ" -> listOf("悲しい", "哀しい", "切ない", "辛い", "泣く", "涙")
            "喜び" -> listOf("嬉しい", "楽しい", "幸福", "ハッピー", "歓喜")
            "怒り" -> listOf("怒る", "腹立つ", "ムカつく", "イライラ", "憤り")
            "恐怖" -> listOf("怖い", "恐ろしい", "恐れる", "ビクビク", "戦慄")
            "愛" -> listOf("愛する", "愛情", "恋", "好き", "慈しむ", "いとしい")
            "憎しみ" -> listOf("憎い", "憎む", "恨む", "嫌い", "嫌悪")
            "嫉妬" -> listOf("妬む", "羨む", "ジェラシー", "やきもち")
            "安心" -> listOf("安らぎ", "穏やか", "落ち着く", "ホッとする")
            "絶望" -> listOf("諦め", "あきらめ", "お先真っ暗", "どん底")
            "空虚" -> listOf("むなしい", "虚しさ", "空っぽ", "何もない")
            "緊張" -> listOf("緊張する", "ドキドキ", "ハラハラ", "ピリピリ")
            "解放" -> listOf("自由", "開放感", "解き放つ", "晴れやか")
            else -> emptyList()
        }

        return (baseWords + synonyms).distinct().filter { it.isNotBlank() }
    }

    /**
     * テーマの類義語・関連語を取得
     */
    fun getThemeSynonyms(theme: String): List<String> {
        return when (theme.lowercase()) {
            "孤独" -> listOf("疎外", "隔絶", "孤立", "一人")
            "期待" -> listOf("予感", "予期", "待機", "希望")
            "喪失" -> listOf("消失", "欠落", "不在", "空白")
            "信頼" -> listOf("依存", "委託", "確信", "安心")
            "不安" -> listOf("動揺", "恐れ", "懸念", "危惧")
            "希望" -> listOf("期待", "願望", "展望", "志向")
            "後悔" -> listOf("悔恨", "残念", "反省", "自責")
            "焦燥" -> listOf("焦り", "苛立ち", "急き", "切迫")
            else -> emptyList()
        }
    }
     ...
     * 表現に含まれる禁止ワードをチェック
     */
    fun checkForbiddenWordsInExpression(expression: String, forbiddenWords: List<String>): List<String> {
